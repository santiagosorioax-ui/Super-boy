...
export const ShopModal: React.FC<ShopModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'sword' | 'drink'>('all');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/75 backdrop-blur-md animate-fade-in">
      ...
      >
        ...
        <div className="p-4 sm:p-5 border-b border-slate-800/80 bg-gradient-to-r from-amber-950/40 via-slate-900 to-indigo-950/40 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-amber-500/20 border border-amber-400/40 flex items-center justify-center text-2xl shadow-inner shadow-amber-500/20">
              🏪
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-xl font-bold bg-gradient-to-r from-amber-200 via-yellow-300 to-amber-400 bg-clip-text text-transparent">
                  Tienda de Santi
                </h2>
                <span className="px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 text-[10px] font-semibold tracking-wide border border-amber-500/30">
                  Plaza Central
                </span>
              </div>
        ...
        <div className="p-3 bg-slate-900/60 border-b border-slate-800/80 flex items-center justify-between gap-2">
          ...
          {inventory.equippedSwordId && (
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-800/80 border border-slate-700 text-xs">
              <span className="text-slate-400">Equipada:</span>
        ...
        <div className="p-4 overflow-y-auto max-h-[60vh] space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {filteredItems.map((item) => {
              const owned = item.category === 'sword' && isSwordOwned(item.id);
              const equipped = item.category === 'sword' && isSwordEquipped(item.id);
        ...
        <div className="p-3 bg-slate-950/80 border-t border-slate-800/80 text-center text-xs text-slate-400 flex items-center justify-center gap-2">
          <span>💡</span>
          <span>
            Las bebidas energéticas tienen efectos temporales acumulables. Las espadas se quedan guardadas para siempre.
          </span>
        </div>
      </div>
    </div>
